...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    if (!student) return fail(res, 'Student not found', 404, 'NOT_FOUND');
...
router.put('/:id', requireRole(['ADMIN', 'TEACHER']), async (req, res, next) => {
  try {
    ...
    if (!updated) return fail(res, 'Student not found', 404, 'NOT_FOUND');
...
router.delete('/:id', requireRole(['ADMIN']), async (req, res, next) => {
  try {
    ...
    if (!deleted) return fail(res, 'Student not found', 404, 'NOT_FOUND');
